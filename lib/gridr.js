...
var Gridr = function Gridr(canvas, grid, transformFunction, renderFunction, viewpoint) {
  ...
  ['click', 'mousedown', 'mouseup', 'mousemove'].forEach(function(elem) {
    cee.on(elem, function(e) {
      var tile = that.getTileForEvent(e.event);
...
Gridr.prototype.fullRedraw = function() {
  ...
  for (var x = Math.floor(viewpoint.x); x <= Math.floor(viewpoint.x + viewpoint.width); x++) {
    for (var y = Math.floor(viewpoint.y); y <= Math.floor(viewpoint.y + viewpoint.height); y++) {
...
Gridr.prototype.draw = function() {
  ...
  for (var i = 0; i < dirty.length; i++) {
    ...
    if (this.isInViewpoint(xCoor, yCoor)) {
      var item = grid.get(xCoor, yCoor);
      item = this._transformer.call(null, item);
      this._render.call(null, tileInfo, ctx, xCoor - viewpoint.x, yCoor - viewpoint.y, item);
    }
  }
...
/**
 * * Returns the coordinates for a cell from a DOM event (eg. click mousemove etc.)
 * */
Gridr.prototype.getTileForEvent = function(e) {
  var canvas = this._canvas;
  var viewpoint = this._viewpoint;
  var tileInfo = Gridr.getTileSize(this._canvas, viewpoint.width, viewpoint.height);
  ...
  if ((e.pageX - canvas.offsetLeft < left) || e.pageY - canvas.offsetTop <
    top) {
    return null;
  }
  var ret = canvasUtils.convertEventCoords(e, canvas);
  ret.x = Math.ceil(ret.x / tileSize - 1 + viewpoint.x);
  ret.y = Math.ceil(ret.y / tileSize - 1 + viewpoint.y);
  return ret;
};
